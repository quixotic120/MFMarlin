...
elevation = 2; // AndrewBCN: was 15mm, decreased to 8mm
...
module wadeidler()
{
    ...
    union()
    {
	 ...
	 translate(idler_axis+[-idler_height/2+2,+idler_long_side/2-idler_long_bottom,0])
	    ...
	    difference() {
	      hull() {
		translate([0,-2,idler_short_side/3-4]) rotate([0,90,0]) cylinder(h=idler_height/2,r=1, $fn=16, center=true);
		translate([0,-2,-idler_short_side/3+4]) rotate([0,90,0]) cylinder(h=idler_height/2,r=1, $fn=16, center=true);
		translate([0,3.8,idler_short_side/3-4]) rotate([0,90,0]) cylinder(h=idler_height/2,r=1, $fn=16, center=true);
		translate([0,3.8,-idler_short_side/3+4]) rotate([0,90,0]) cylinder(h=idler_height/2,r=1, $fn=16, center=true);
	      }
	    ...
	    hull() {
	      translate([idler_height/4-2.3,3.8,idler_short_side/3-4.1]) sphere(r=1, $fn=16);
	      translate([idler_height/4-2.3,3.8,-idler_short_side/3+4.1]) sphere(r=1, $fn=16);
	    }
	...
	difference()
	{
		...
		translate(idler_axis+[2-idler_height,0,0])
		{
			...
			rotate([0,90,0])
			{
				...
				cube([idler_mounting_hole_diameter,idler_mounting_hole_elongation+5,
					idler_height+2]);
			}
